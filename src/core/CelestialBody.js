...
export default {
	...
	//绑定轨道要素 OrbitalElements（比如地球的半长轴、偏心率、倾角等）
	//relativeTo 表示绕谁转（比如月球相对地球）
	init(universe) {
		this.reset();
	...
	//根据儒略日 jd 计算天体位置。
	// 如果是中心天体（如太阳）→ 位置是 (0,0,0)。
	// 否则调用 orbitalElements.calculatePosition()
	...
	*/
	...
	//每次 tick 更新速度、位置、轨道角度
	// 检查是否完成一圈（触发 onRevolution 回调）
