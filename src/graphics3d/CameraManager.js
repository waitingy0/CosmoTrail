...
export default class CameraManager {
	...
	toggleCamera = () => {
		...
		if (lookFromBody) {
			...
			if (this.viewSettings.lookFrom === ORBITAL_CAMERA_TYPE) {
				...
				if (this.currentCamera.CosmoTrail && this.currentCamera.CosmoTrail.controls) this.currentCamera.CosmoTrail.controls.enabled = true;
			}
		}
	...
	updateCamera() {
		...
		const lookFromBody = this.bodies3d[this.viewSettings.lookFrom];
		const lookAtBody = this.bodies3d[this.viewSettings.lookAt];
		const controls = this.currentCamera.CosmoTrail && this.currentCamera.CosmoTrail.controls;
	...
	getNewCamera(isOrbital) {
		...
		if (isOrbital) {
			...
			cam.CosmoTrail = cam.CosmoTrail || {};
			cam.CosmoTrail.controls = controls;
	...
	disableControls() {
		this.allCameras.forEach(cam => {
			if (cam.CosmoTrail && cam.CosmoTrail.controls) cam.CosmoTrail.controls.enabled = false;
		});
	}
